...
class Player:
    ...
    def move(self, new_coordinates: tuple) -> None:
        '''Exchange the old coordinates of the Player self for the new
...
class UserPlayer(Player):
    '''
    Create a Player for a user that can play MazeGame
    '''
    
    def get_direction(self):
        '''Return the direction that the user wishes to move their
        UserPlayer self.
        '''
        user_command = input(("It's your turn {0}! Where would you like to" \
                             + " go? (N, S, E, W): ").format(self.name))

        valid_inputs = ['N', 'S', 'E', 'W']
        if user_command in valid_inputs:
            return user_command
        else:
            raise InvalidMoveError('You may only move N, S, E, or W')

class ComputerPlayer(Player):
    '''
    Create a Player for a computer that can play MazeGame
    '''

    def get_direction(self):
        '''
        Returns the direction randomly selected by the ComputerPlayer self.
        '''
        direction_list = ['N', 'S', 'E', 'W']
        computer_command = direction_list[random.randint(0, 3)]
...
#It seems that there will be 2 subclasses of the superclass Player:
#   ComputerPlayer and UserPlayer subclasses
#Players have the methods: move, get_direction (this one is different for
#   both types of user, meaning each subclass should have their own
